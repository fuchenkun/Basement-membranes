...
setwd("C:\Users\HASEE\Desktop\BMs\41.heatmap")       
...
up=t(up)
#设置颜色
colVector=c("blue","white","red","gray")
...
rbPal1 <- colorRampPalette(colors = c(colVector[1],"white",colVector[3]))
...
rbPal2 <- colorRampPalette(colors = c(colVector[2],colVector[4]))
...
plot(1,xlim=c(1,length(unique(x))+1),ylim=c(1,length(unique(y))+1),
     ...
     main = "Coexpression across cancer types(MMP7)",cex.main=2)
#填充颜色
